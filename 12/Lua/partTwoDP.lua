...
local function solve(mapTable, damagedTable)
    ...
    local x = table.concat(mapTable, "")
    local y = table.concat(damagedTable, ",")
    if solveDP[y] == nil then
        solveDP[y] = {}
    end
    if solveDP[y][x] ~= nil then
        return solveDP[y][x]
    end
    local sum = 0
...
for line in io.lines("input.txt") do
    ...
    map = string.rep(map, 5, '?')
    damagedList = string.rep(damagedList, 5, ",")
    solveDP = {}
    local mapTable = {}
    local damagedTable = {}
    local count = 0
