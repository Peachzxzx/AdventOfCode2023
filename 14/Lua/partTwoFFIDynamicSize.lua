...
for line in io.lines("input.txt") do
    keepAwayFromGC[loopIndex] = line
    loopIndex = loopIndex + 1
end

local height = #keepAwayFromGC
local width = #keepAwayFromGC[1]
local map = ffi.new("char *[?]", height)
for i = 0, height - 1 do
    local cString = ffi.new("char[?]", width + 1)
    ffi.copy(cString, keepAwayFromGC[i + 1])
    map[i] = cString
